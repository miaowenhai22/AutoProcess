...
public class WasDecoder {
	...
	public void load(InputStream in) throws Exception {
		...
		if (!"SP".equals(flag)) {
			throw new Exception("鏂囦欢澶存爣蹇楅敊璇�:" + flag);
		}
		...
		if (len < 0)
			throw new Exception("甯у欢鏃朵俊鎭敊璇�: " + len);
		for (int i = 0; i < len; i++) {
	...
	public static WasImage loadImage(InputStream is) throws IOException {
		...
		if (!"SP".equals(new String(buf, 0, 2))) {
			in.close();
			throw new IllegalArgumentException("非WAS文件格式!");
		}
		imageHeaderSize = in.readUnsignedShort();
		int len = imageHeaderSize - 12;
		if (len < 0) {
			in.close();
			throw new IllegalArgumentException("帧延时信息错误！");
		}
